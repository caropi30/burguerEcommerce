...
import { useGetCategoriesQuery, useGetUserQuery } from '../services/burgersApi'
...
const HomeScreen = () => {
    ...
    const renderContent = () => (
        <View style={styles.container}>
            <Text style={styles.userTxt}>Hola! {userData?.name}</Text>
            <View style={styles.info}>
                <Text style={styles.title}>¿Qué te provoca hoy?</Text>
                <Text style={styles.paragraph}>
                    Escoge entre nuestras opciones individuales y combos
                </Text>
            </View>
            <>
                {subcategories.map((item) => (
                    <Card
                        ...
                        icon={item.icon}
                        onPress={() =>
                            handleCategoryFilter(item.title, item.subcategories)
                        }
                    />
                ))}
            </>
    ...
    return (
        <>
            <Header isHome isLoading={isLoading} />
